...
# Assuming your TTS and STC imports are available
from glados.ASR.models.TTS import tts_glados, tts_kokoro  # Adjust path

from .glados_config import AudioMessage  # Adjust path
        ...
        while not self.shutdown_event.is_set():
            try:
                ...
                if text_to_speak == "<EOS>":
                    ...
                    self.audio_output_queue.put(AudioMessage(audio=np.array([], dtype=np.float32), text="", is_eos=True))
                    continue
                
                if not text_to_speak.strip(): # Check for empty or whitespace-only strings
                    logger.warning(f"TTS Synthesizer: Received empty or whitespace string: '{text_to_speak}'")
                    continue # Don't process empty strings

                logger.info(f"TTS Synthesizer: Processing text for TTS: '{text_to_speak}'")
                ...
                audio_duration = 0
                if len(audio_data) > 0 and self.tts_model.sample_rate > 0:
