...
class MelSpectrogramConfig(BaseModel):
    ...
    normalize: Literal["per_feature", "all_features"] | None = Field(
        "per_feature", description="Normalization method ('per_feature', 'all_features', or None to disable)."
    )
    ...
    mel_norm: Literal["slaney", "htk"] = Field(
        "slaney",
        description="Mel filterbank normalization type ('slaney' or 'htk'). Manual implementation primarily targets 'slaney'.",
    )
...
class MelSpectrogramCalculator:
    ...
    def __init__(
        ...
        **kwargs: float,  # To absorb any other potential NeMo args from config
        ...
        if self.win_length > self.n_fft:
            raise ValueError(
                f"Window length ({self.win_length}) from window_size {window_size}s cannot be greater than n_fft ({self.n_fft})."
            )
    ...
    def compute(self, audio: NDArray[np.float32]) -> NDArray[np.float32]:
        ...
        if self.exact_pad:
            ...
            audio_padded = np.pad(audio_preemph, (self.stft_pad_amount, self.stft_pad_amount), mode="reflect")
        else:
